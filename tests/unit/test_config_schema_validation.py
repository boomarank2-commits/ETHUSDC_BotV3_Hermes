...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("portfolio", "lot_notional_usdc", 50),
        ("portfolio", "default_deployment_budget_usdc", 200),
        ("portfolio", "allowed_deployment_budgets_usdc", [100, 200, 500]),
        ("portfolio", "compounding_enabled", True),
        ("portfolio", "baseline_fee_bps_per_side", 0),
        ("portfolio", "baseline_slippage_bps_per_side", 0),
        ("portfolio", "soft_drawdown_fraction", 0.16),
        ("target_guidance", "used_for_candidate_search", True),
        ("target_guidance", "includes_zero_trade_days", False),
        ("shadow", "orders_enabled", True),
        ("shadow", "trading_api_enabled", True),
        ("shadow", "api_keys_used", True),
        ("shadow", "automatic_live_transition_enabled", True),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("safety", "live_enabled"),
        ("portfolio", "lot_notional_usdc"),
        ("target_guidance", "measurement"),
        ("shadow", "orders_enabled"),
        ("phase", "current"),
    ],
)
