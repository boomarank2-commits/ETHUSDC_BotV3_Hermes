...
def _selection_basis(window: SelectionTrainingWindow, config: FrozenSelectionConfig) -> dict[str, Any]:
    w = window.to_dict()
    c = config.to_dict()
    if c["fold_identity"] != w["fold_identity"]:
        raise InnerSelectionError("selection config and training window use different Task-14 plans")
    for row in c["candidate_evidence"]:
        if row["training_window_sha256"] != window.window_sha256:
            raise InnerSelectionError("candidate evidence belongs to a different training window")
    ...
    if support["pbo"]["state"] == COMPLETE and support["pbo"]["value"] > _MAX_PBO:
    ...
    dsr_values = support["dsr"]["value"] if support["dsr"]["state"] == COMPLETE else {}
    for row in c["candidate_evidence"]:
        candidate_id = row["candidate"]["canonical_candidate_id"]
        gate = evaluate_quality_gates(row["quality_evidence"], stage="selection").to_dict()
        rank = _ranking_row(row)
        dsr_score = dsr_values.get(candidate_id) if isinstance(dsr_values, dict) else None
        gate_passed = gate["passed"] is True and gate["status"] == "pass"
        dsr_passed = isinstance(dsr_score, (int, float)) and not isinstance(dsr_score, bool) and math.isfinite(float(dsr_score)) and float(dsr_score) >= _MIN_DSR
        ranking_rows.append(
            {
                **rank,
        ...
        if not gate_passed:
            candidate_rejections.add(f"QUALITY_GATE_NOT_PASSED:{candidate_id}:{gate['status']}")
        if support["dsr"]["state"] == COMPLETE and not dsr_passed:
            candidate_rejections.add(f"DEVELOPMENT_DSR_GATE_FAILED:{candidate_id}")
        if gate_passed and dsr_passed:
    ...
    if not blockers and eligible:
        ...
        outcome = CANDIDATE
    elif not eligible and support["mode"] == SYNTHETIC_TEST_FIXTURE:
