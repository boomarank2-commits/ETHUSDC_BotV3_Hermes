...
CANDIDATE_PENDING_SCHEMA: Final = (
    "protocol_v3_candidate_identity_pending_task15_v1"
)
...
_TRANSACTION_ID = re.compile(
    r"^protocol_v3_transaction_sha256:[0-9a-f]{64}$"
)
_CHECKPOINT_ID = re.compile(
    r"^protocol_v3_checkpoint_sha256:[0-9a-f]{64}$"
)
...
CANONICAL_CONTRACT: dict[str, Any] = {
    ...
    "lock_policy": {
        ...
        "blind_stale_overwrite_forbidden": True,
    ...
    "fault_injection_phases": [
        "before_checkpoint_temp",
        "after_checkpoint_temp_fsync",
        "after_checkpoint_temp_validate",
        "after_checkpoint_replace",
        "after_checkpoint_reload",
        "after_ledger_reuse_append",
        "before_head_replace",
        "after_head_replace",
    ],
    "deferred_scope": {
...
class IdentitySlot:
    ...
    @property
    def name(self) -> str:
        return str(self.to_dict()["name"])

    @property
    def state(self) -> str:
        return str(self.to_dict()["state"])
...
class TransactionIdentity:
    ...
    def work_unit_id(self) -> str:
        return str(self.to_dict()["work_unit_id"])

    @property
    def cache_key(self) -> str:
        return f"{CACHE_PREFIX}:{self.identity_sha256}"
...
class TransactionCheckpoint:
    ...
    def sequence(self) -> int:
        return int(self.to_dict()["sequence"])
...
def load_transaction_contract(
    ...
    *,
    ...
    path = (
        ...
        else root / TRANSACTION_CONTRACT_PATH
    )
    ...
    try:
        ...
        raise ProtocolV3TransactionError(
            f"Protocol v3 transaction contract is missing or invalid: {path}"
        ) from exc
...
def validate_transaction_contract(value: Mapping[str, Any]) -> None:
    if not isinstance(value, Mapping) or normalize(value) != CANONICAL_CONTRACT:
        raise ProtocolV3TransactionError(
            "Protocol v3 transaction contract is not canonical"
        )


def build_bound_identity_slot(
    name: str,
    schema_version: str,
    ...
    if not normalized:
        raise ProtocolV3TransactionError(
            "BOUND identity slots require a non-empty payload"
        )
    return _build_slot(name, BOUND, schema_version, normalized, "bound")


def build_genesis_identity_slot(
    name: str,
    schema_version: str,
    reason: str,
) -> IdentitySlot:
...
def build_not_applicable_identity_slot(
    name: str,
    schema_version: str,
    reason: str,
) -> IdentitySlot:
...
def _build_slot(
    ...
    basis = {
        ...
        "name": required_slot_name(name),
        "state": state,
        "identity_schema": schema_identifier(
            schema,
            f"{name}.identity_schema",
        ),
        "payload": normalize(payload),
        "reason": reason,
    }
    return validate_identity_slot(
        {**basis, "slot_sha256": digest(basis)}
    )


def validate_identity_slot(
    value: IdentitySlot | Mapping[str, Any],
) -> IdentitySlot:
    root = (
        value.to_dict()
        if isinstance(value, IdentitySlot)
        else dict(require_mapping(value, "identity_slot"))
    )
    exact_keys(
        root,
        {
    ...
    literal(
        root,
        "schema_version",
        IDENTITY_SLOT_SCHEMA_VERSION,
        "identity_slot",
    )
    name = required_slot_name(root.get("name"))
    state = root.get("state")
    if state not in {BOUND, GENESIS, NOT_APPLICABLE}:
        raise ProtocolV3TransactionError(
            f"identity slot state is invalid: {name}"
        )
    schema_identifier(root.get("identity_schema"), f"{name}.identity_schema")
    payload = dict(require_mapping(root.get("payload"), f"{name}.payload"))
    reason = required_text(root.get("reason"), f"{name}.reason")
    if state == BOUND and (not payload or reason != "bound"):
        raise ProtocolV3TransactionError(
            f"BOUND identity slot is incomplete: {name}"
        )
    if state != BOUND and payload:
        raise ProtocolV3TransactionError(
            f"typed non-bound slot payload must be empty: {name}"
        )
    reject_raw(payload, f"{name}.payload")
    finite_json(payload, f"{name}.payload")
    observed = sha256(root.get("slot_sha256"), f"{name}.slot_sha256")
    digest_basis = dict(root)
    digest_basis.pop("slot_sha256")
    if observed != digest(digest_basis):
        raise ProtocolV3TransactionError(
            f"identity slot digest mismatch: {name}"
        )
    return IdentitySlot(canonical(root), observed)


def build_sealed_store_heads_slot(
    index_paths: Sequence[str | Path],
    repository_root: str | Path,
) -> IdentitySlot:
    repo = repo_root(repository_root)
    if (
        isinstance(index_paths, (str, bytes, bytearray))
        or not isinstance(index_paths, Sequence)
    ):
        raise ProtocolV3TransactionError(
            "sealed store index paths must be a sequence"
        )
    if not index_paths:
        return build_genesis_identity_slot(
            STORE_HEADS_SLOT,
            STORE_HEADS_SCHEMA,
            "no_protocol_v3_artifact_index_committed",
        )
    heads = sorted(
        (_artifact_head(path, repo) for path in index_paths),
        key=lambda row: row["relative_path"],
    )
    if len({row["relative_path"] for row in heads}) != len(heads):
        raise ProtocolV3TransactionError(
            "sealed artifact index paths must be unique"
        )
    return build_bound_identity_slot(
        STORE_HEADS_SLOT,
        STORE_HEADS_SCHEMA,
        {"indexes": heads},
    )
...
def build_transaction_identity(
    *,
    ...
    work_unit_id: str,
    candidate_identity: IdentitySlot,
    fold_identity: IdentitySlot,
    rotation_state_identity: IdentitySlot,
    sealed_store_heads: IdentitySlot,
    repository_root: str | Path,
) -> TransactionIdentity:
    validate_run_fingerprint(run_fingerprint)
    validate_context_parity_binding(context_binding)
    if not isinstance(horizon_policy, HorizonPolicy):
        raise ProtocolV3TransactionError(
            "horizon_policy must be a validated HorizonPolicy"
        )
    run = run_fingerprint.payload()
    runtime = normalize(run["context"]["runtime_binding"])
    observed_runtime = {
        ...
        "cache_key": context_binding.cache_key,
        "resume_key": context_binding.resume_key,
    }
    if runtime != observed_runtime:
        raise ProtocolV3TransactionError(
            "run fingerprint and concrete ContextParityBinding differ"
        )
    supplied = {
    ...
    slots = {
        RAW_DATA_SLOT: build_bound_identity_slot(
            RAW_DATA_SLOT,
            "protocol_v3_run_raw_data_identity_v2",
            run["raw_data"],
        ),
        CODE_PIPELINE_SLOT: build_bound_identity_slot(
            CODE_PIPELINE_SLOT,
            "protocol_v3_code_pipeline_identity_v1",
            {"code": run["code"], "pipeline": run["pipeline"]},
        ),
        FEATURE_SLOT: build_bound_identity_slot(
            FEATURE_SLOT,
            "protocol_v3_feature_component_identity_v1",
            run["features"],
        ),
        CONTEXT_SLOT: build_bound_identity_slot(
            CONTEXT_SLOT,
            "protocol_v3_context_runtime_identity_v2",
            runtime,
        ),
        BOUNDARY_SLOT: build_bound_identity_slot(
            BOUNDARY_SLOT,
            "protocol_v3_boundary_component_identity_v1",
            run["boundary"],
        ),
        HORIZON_SLOT: build_bound_identity_slot(
            HORIZON_SLOT,
            "protocol_v3_horizon_policy_identity_v1",
            horizon,
        ),
        EXECUTION_SLOT: build_bound_identity_slot(
            EXECUTION_SLOT,
            "protocol_v3_execution_identity_v1",
            {
                "execution_parity_contract_version": (
                    EXECUTION_PARITY_CONTRACT_VERSION
                ),
                ...
                "simulator_source_sha256": run["simulator"][
                    "source_sha256"
                ],
            },
        ),
        SIMULATOR_SLOT: build_bound_identity_slot(
            SIMULATOR_SLOT,
            "protocol_v3_simulator_component_identity_v1",
            run["simulator"],
        ),
        COST_SLOT: build_bound_identity_slot(
            COST_SLOT,
            "protocol_v3_cost_component_identity_v1",
            run["cost_model"],
        ),
        QUALITY_SLOT: build_bound_identity_slot(
            QUALITY_SLOT,
            "protocol_v3_quality_gate_component_identity_v1",
            run["quality_gates"],
        ),
        EXCHANGE_SLOT: build_bound_identity_slot(
            EXCHANGE_SLOT,
            "protocol_v3_exchange_info_identity_v1",
            run["exchange_info"],
        ),
        TRIAL_LEDGER_SLOT: build_bound_identity_slot(
            TRIAL_LEDGER_SLOT,
            "protocol_v3_trial_ledger_decision_head_v1",
            run["trial_ledger_head"],
        ),
        **supplied,
    }
    basis = {
        "schema_version": TRANSACTION_IDENTITY_SCHEMA_VERSION,
        "protocol_version": PROTOCOL_VERSION,
        "contract_version": TRANSACTION_CONTRACT_VERSION,
        "work_unit_id": safe_id(work_unit_id, "work_unit_id"),
        "run_fingerprint": run_fingerprint.to_dict(),
        "context_binding": observed_runtime,
        "identity_slots": [
            slots[name].to_dict() for name in REQUIRED_IDENTITY_SLOTS
        ],
        "safety": SAFETY,
    }
    identity_sha = digest(basis)
    return validate_transaction_identity(
        {
            **basis,
            "identity_sha256": identity_sha,
            "transaction_id": f"{TRANSACTION_PREFIX}:{identity_sha}",
        },
...
def validate_transaction_identity(
    value: TransactionIdentity | Mapping[str, Any],
    *,
    repository_root: str | Path,
) -> TransactionIdentity:
    root = (
        value.to_dict()
        if isinstance(value, TransactionIdentity)
        else dict(require_mapping(value, "transaction_identity"))
    )
    exact_keys(
        root,
        {
    ...
    run = dict(
        require_mapping(
            root.get("run_fingerprint"),
            "transaction_identity.run_fingerprint",
        )
    )
    validate_run_fingerprint(run)
    runtime = dict(
        require_mapping(
            root.get("context_binding"),
            "transaction_identity.context_binding",
        )
    )
    if normalize(run["context"]["runtime_binding"]) != normalize(runtime):
        raise ProtocolV3TransactionError(
            "transaction context binding differs from run fingerprint"
        )
    raw_slots = root.get("identity_slots")
    if not isinstance(raw_slots, list):
        raise ProtocolV3TransactionError(
            "transaction identity slots must be a list"
        )
    slots = [validate_identity_slot(row) for row in raw_slots]
    if [slot.name for slot in slots] != list(REQUIRED_IDENTITY_SLOTS):
        raise ProtocolV3TransactionError(
            "transaction identity slots are missing, extra, or reordered"
        )
    ...
    if root.get("safety") != SAFETY:
        raise ProtocolV3TransactionError(
            "transaction identity safety locks are invalid"
        )
    observed = sha256(
        root.get("identity_sha256"),
        "transaction_identity.identity_sha256",
    )
    tx_id = transaction_id(root.get("transaction_id"))
    digest_basis = dict(root)
    digest_basis.pop("identity_sha256")
    digest_basis.pop("transaction_id")
    expected = digest(digest_basis)
    if (
        observed != expected
        or tx_id != f"{TRANSACTION_PREFIX}:{expected}"
    ):
        raise ProtocolV3TransactionError(
            "transaction identity digest or id mismatch"
        )
    return TransactionIdentity(canonical(root), expected, tx_id)


def _validate_derived_slots(
    run: Mapping[str, Any],
    ...
    for name, payload in expected.items():
        ...
        if row["state"] != BOUND or normalize(row["payload"]) != normalize(
            payload
        ):
            raise ProtocolV3TransactionError(
                f"derived identity slot differs from run fingerprint: {name}"
            )
    ...
    if (
        ...
        raise ProtocolV3TransactionError(
            "execution identity slot is not canonical"
        )
...
def _validate_fold_slot(
    ...
    if row["state"] != BOUND:
        raise ProtocolV3TransactionError(
            "fold_identity must be BOUND to an exact Task-14 plan"
        )
    if row["identity_schema"] != FOLD_IDENTITY_SCHEMA:
        raise ProtocolV3TransactionError(
            "fold_identity schema is not the Task-14 schema"
        )
    if row["reason"] != "bound":
        raise ProtocolV3TransactionError("fold_identity reason is invalid")
    try:
        normalized = validate_inner_fold_identity_payload(
            row["payload"],
            repo_root=repository_root,
        )
    except Exception as exc:
        raise ProtocolV3TransactionError(
            f"fold_identity is not a valid Task-14 plan: {exc}"
        ) from exc
...
def _validate_horizon_slot(slot: IdentitySlot) -> HorizonPolicy:
    ...
    if row["state"] != BOUND:
        raise ProtocolV3TransactionError("horizon identity must be BOUND")
    exact_keys(
        payload,
        {
            "contract_version",
        ...
        "horizon_identity.payload",
    )
    policy = HorizonPolicy(
        payload["max_label_horizon_minutes"],
        payload["max_holding_period_minutes"],
        payload["pending_entry_latency_minutes"],
        payload["execution_bar_minutes"],
    )
    if payload != {
        **policy.basis(),
...
def _validate_store_heads_slot(
    ...
    row = slot.to_dict()
    if row["identity_schema"] != STORE_HEADS_SCHEMA:
        raise ProtocolV3TransactionError(
            "sealed store-head schema is invalid"
        )
    if row["state"] == GENESIS:
        if row["reason"] != "no_protocol_v3_artifact_index_committed":
            raise ProtocolV3TransactionError(
                "sealed store-head genesis reason is invalid"
            )
        return
    if row["state"] != BOUND:
        raise ProtocolV3TransactionError(
            "sealed store heads cannot be NOT_APPLICABLE"
        )
    payload = dict(row["payload"])
    exact_keys(payload, {"indexes"}, "sealed_store_heads.payload")
    if (
        validate_artifact_heads(payload["indexes"], repository_root)
        != payload["indexes"]
    ):
        raise ProtocolV3TransactionError(
            "sealed store-head payload is not canonical"
        )


def _artifact_head(index_path: str | Path, repo: Path) -> dict[str, Any]:
    bundle = read_compact_artifact_bundle(index_path, repo)
    path = Path(index_path)
    if not path.is_absolute():
        path = repo / path
    relative = path.resolve(strict=True).relative_to(repo).as_posix()
    payload = bundle.index.to_dict()
    return {
        "relative_path": relative,
        "index_id": payload["index_id"],
        "index_sha256": payload["index_sha256"],
        "parent_report_id": payload["parent_report"]["report_id"],
        "work_unit_id": payload["work_unit"]["work_unit_id"],
    }
...
def validate_artifact_heads(
    ...
    if not isinstance(value, list):
        raise ProtocolV3TransactionError(
            "artifact index heads must be a list"
        )
    repo = repo_root(repository_root)
    normalized: list[dict[str, Any]] = []
    for index, raw in enumerate(value):
        row = dict(require_mapping(raw, f"artifact_indexes[{index}]"))
        exact_keys(
            row,
            {
                "relative_path",
            ...
            f"artifact_indexes[{index}]",
        )
        relative = safe_relative(
            row["relative_path"],
            f"artifact_indexes[{index}].relative_path",
        )
        if not str(relative).startswith(f"{ARTIFACT_INDEX_ROOT}/"):
            raise ProtocolV3TransactionError(
                "artifact index head is outside the Task-12 index root"
            )
        if row != _artifact_head(repo.joinpath(*relative.parts), repo):
            raise ProtocolV3TransactionError(
                "artifact index head differs from revalidated index"
            )
    ...
    if normalized != sorted(
        ...
        raise ProtocolV3TransactionError(
            "artifact index heads must be canonically sorted"
        )
    if len({row["relative_path"] for row in normalized}) != len(normalized):
        raise ProtocolV3TransactionError(
            "artifact index heads must be unique"
        )
    return normalized


def store_heads_from_identity(
...
def build_seed_state(
    ...
    *,
    ...
    basis = {
        "schema_version": "protocol_v3_checkpoint_seed_state_v1",
        "namespace": (
...
def validate_seed_state(
    ...
    root = dict(require_mapping(value, "seed_state"))
    exact_keys(
        root,
        {
            "schema_version",
    ...
    literal(
        root,
        "schema_version",
        "protocol_v3_checkpoint_seed_state_v1",
        "seed_state",
    )
    expected = build_seed_state(
        manifest,
        origin_index=positive_int(
            root["origin_index"],
            "seed_state.origin_index",
        ),
        cycle_index=positive_int(
            root["cycle_index"],
            "seed_state.cycle_index",
        ),
        stage=required_text(root["stage"], "seed_state.stage"),
    )
    if root != expected:
        raise ProtocolV3TransactionError(
            "seed state does not match the frozen pre-run manifest"
        )
    return root
...
def build_stop_state(
    *,
    ...
    reason = stagnation_stop_reason(
        completed_cycles=completed_cycles,
        consecutive_non_improving_cycles=consecutive_non_improving_cycles,
    )
    basis = {
        "schema_version": "protocol_v3_checkpoint_stop_state_v1",
        "completed_cycles": completed_cycles,
        "consecutive_non_improving_cycles": (
            consecutive_non_improving_cycles
        ),
        "stop_reason": reason or "NOT_STOPPED",
    }
    return {**basis, "stop_state_sha256": digest(basis)}


def validate_stop_state(value: Mapping[str, Any]) -> dict[str, Any]:
    root = dict(require_mapping(value, "stop_state"))
    exact_keys(
        root,
        {
            "schema_version",
    ...
    literal(
        root,
        "schema_version",
        "protocol_v3_checkpoint_stop_state_v1",
        "stop_state",
    )
    expected = build_stop_state(
        completed_cycles=nonnegative_int(
            root["completed_cycles"],
            "stop_state.completed_cycles",
        ),
        consecutive_non_improving_cycles=nonnegative_int(
            root["consecutive_non_improving_cycles"],
            "stop_state.consecutive_non_improving_cycles",
        ),
    )
    if root != expected:
        raise ProtocolV3TransactionError(
            "stop/stagnation state is not canonical"
        )
    return root
...
def budget_mapping(usage: BudgetUsage) -> dict[str, Any]:
    ...
    return {
        "cycles_by_origin": list(usage.cycles_by_origin),
        "reserved_generated": usage.reserved_generated,
        "reserved_tested": usage.reserved_tested,
        "reserved_walk_forward": usage.reserved_walk_forward,
        "reserved_finalists": usage.reserved_finalists,
    }
...
def validate_budget_mapping(value: Mapping[str, Any]) -> None:
    ...
    if not isinstance(cycles, list) or len(cycles) != 12:
        raise ProtocolV3TransactionError(
            "budget usage must contain twelve origin counters"
        )
    validate_budget_usage(
        BudgetUsage(
            tuple(cycles),
            value["reserved_generated"],
            value["reserved_tested"],
            value["reserved_walk_forward"],
            value["reserved_finalists"],
        )
    )
...
def manifest_from_mapping(value: Mapping[str, Any]) -> PreRunManifest:
    raw = dict(value)
    digest_value = raw.pop("manifest_sha256", None)
    if not isinstance(digest_value, str):
        raise ProtocolV3TransactionError(
            "pre-run manifest digest is missing"
        )
    manifest = PreRunManifest(canonical(raw), digest_value)
    validate_pre_run_manifest(manifest)
    return manifest
...
def canonical(value: Any) -> str:
    try:
        ...
        raise ProtocolV3TransactionError(
            f"value is not strict JSON: {exc}"
        ) from exc
...
def require_mapping(value: Any, path: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise ProtocolV3TransactionError(f"{path} must be an object")
    return value
...
def required_slot_name(value: Any) -> str:
    if value not in REQUIRED_IDENTITY_SLOTS:
        raise ProtocolV3TransactionError(
            f"identity slot name is invalid: {value!r}"
        )
    return str(value)
...
def schema_identifier(value: Any, path: str) -> str:
    if not isinstance(value, str) or not _SCHEMA_ID.fullmatch(value):
        raise ProtocolV3TransactionError(
            f"{path} is not a valid schema identifier"
        )
    return value
...
def transaction_id(value: Any) -> str:
    if not isinstance(value, str) or not _TRANSACTION_ID.fullmatch(value):
        raise ProtocolV3TransactionError("transaction_id is invalid")
    return value


def checkpoint_id(value: Any) -> str:
    if not isinstance(value, str) or not _CHECKPOINT_ID.fullmatch(value):
        raise ProtocolV3TransactionError("checkpoint_id is invalid")
    return value


def cache_id(value: Any) -> str:
    if not isinstance(value, str) or not _CACHE_ID.fullmatch(value):
        raise ProtocolV3TransactionError("cache_record_id is invalid")
    return value
...
def safe_relative(value: Any, path: str) -> PurePosixPath:
    if not isinstance(value, str) or not value or "\\" in value:
        raise ProtocolV3TransactionError(
            f"{path} must be a POSIX relative path"
        )
    relative = PurePosixPath(value)
    if relative.is_absolute() or ".." in relative.parts or "." in relative.parts:
        raise ProtocolV3TransactionError(f"{path} escapes its root")
    return relative
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            if str(key).lower() in _FORBIDDEN_RAW_KEYS:
                raise ProtocolV3TransactionError(
                    f"{path} embeds forbidden raw market data"
                )
            reject_raw(item, f"{path}.{key}")
    elif isinstance(value, list):
        for index, item in enumerate(value):
            reject_raw(item, f"{path}[{index}]")
...
__all__ = [
    ...
    "CACHE_ROOT",
    "CHECKPOINT_HEAD_SCHEMA_VERSION",
    "CHECKPOINT_ROOT",
    ...
    "CacheRecord",
    "IdentitySlot",
    "ProtocolV3TransactionError",
    "TransactionCheckpoint",
    "TransactionIdentity",
    "TransactionLock",
    "build_bound_identity_slot",
